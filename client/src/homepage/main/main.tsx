...
const Main = () => {
  ...
  async function afterSignIn() {
    ...
    if (profile && profile.id && profile.email) {
      const res = await Api.getCustomer(profile.id);
  ...
  async function tokenTester() {
    ...
    fetch(`http://localhost:8072/hotel-service/bookings/customer/673d4aff-9c1d-4b69-bdc0-c2ab0aed12c9`, {
      method: 'GET',
      headers: {
      ...
      <div className='imageColumn'>
        <div className='imageColumn_WRAPPER'>
          <div className='imageColumn_AUTH'>
            <a style={{ cursor: 'pointer' }} onClick={() => mockLogin()} title='Sign in'>Sign in</a>
            <a style={{ cursor: 'pointer' }} onClick={() => mockTest()} title='Test keycloak'>Z</a>
            <a style={{ cursor: 'pointer' }} onClick={() => fetchCustomers()} title='Fetch customers'>A</a>
            <a style={{ cursor: 'pointer' }} onClick={() => testBooking()} title='Create booking'>B</a>
            <a style={{ cursor: 'pointer' }} onClick={() => testInvoice()} title='Create invoice'>G</a>
            <a style={{ cursor: 'pointer' }} onClick={() => afterSignIn()} title='Create customer'>F</a>
            <a style={{ cursor: 'pointer' }} onClick={() => fetchBookings()} title='Fetch customer bookings'>X</a>
            <a style={{ cursor: 'pointer' }} onClick={() => fetchInvoices()} title='Fetch customer invoices'>XX</a>
            <a style={{ cursor: 'pointer' }} onClick={() => tokenTester()} title='Test if token is valid'>XXX</a>
            <a style={{ cursor: 'pointer' }} onClick={() => isProfileExistent()} title='Does profile exist function'>E</a>
            <a style={{ cursor: 'pointer' }} onClick={() => authSpec()} title='see api.authToken'>C</a>
          </div>
